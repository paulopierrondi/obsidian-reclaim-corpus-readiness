...
// src/scanner.ts
var DEFAULT_SHINGLE_SIZE = 8;
var DEFAULT_NUM_HASHES = 32;
var DEFAULT_LSH_BANDS = 8;
var DEFAULT_NEAR_DUP_THRESHOLD = 0.72;
function fnv1a32(s) {
  let hash = 2166136261;
...
function findNearDuplicates(docs, shingleSize, numHashes, nearDupThreshold) {
  ...
  const lshBands = DEFAULT_LSH_BANDS;
  const rows = Math.max(1, Math.floor(numHashes / lshBands));
  const buckets = /* @__PURE__ */ new Map();
  for (let di = 0; di < withSigs.length; di++) {
    const sig = signatures[di];
    if (sig.length === 0)
      continue;
    for (let band = 0; band < lshBands; band++) {
...
function exportMarkdownReport(result, vaultName) {
  ...
  if (result.subScores) {
    ...
    lines.push("|---|---|---|");
    lines.push(`| Uniqueness | ${result.subScores.uniqueness}/100 | 30% |`);
    lines.push(`| Structure | ${result.subScores.structure}/100 | 25% |`);
    lines.push(`| Density | ${result.subScores.density}/100 | 20% |`);
    lines.push(`| Title Quality | ${result.subScores.titleQuality}/100 | 15% |`);
    lines.push(`| Freshness | ${result.subScores.freshness}/100 | 10% |`);
    lines.push("");
  }
  lines.push("## Stats");
  lines.push("");
...
async function scanVault(vault, options = {}) {
  var _a, _b, _c, _d;
  const shingleSize = (_a = options.shingleSize) != null ? _a : DEFAULT_SHINGLE_SIZE;
  const numHashes = (_b = options.numHashes) != null ? _b : DEFAULT_NUM_HASHES;
  const nearDupThreshold = (_c = options.nearDupThreshold) != null ? _c : DEFAULT_NEAR_DUP_THRESHOLD;
  ...
  for (const file of files) {
    ...
    const mtime = (_d = stat == null ? void 0 : stat.mtime) != null ? _d : Date.now();
    const tokens = tokenize(normalized);
    const shingles = shingleSet(tokens, shingleSize);
    const signature = shingles.size > 0 ? minhashSignature(shingles, buildHashCoeffs(numHashes)) : null;
    docs.push({
      path: file.path,
      name: file.name,
  ...
  const nearDuplicates = findNearDuplicates(docs, shingleSize, numHashes, nearDupThreshold);
...
var ReclaimPlugin = class extends import_obsidian.Plugin {
  ...
  scanOptions() {
    return {
      nearDupThreshold: this.settings.nearDupThreshold,
      shingleSize: this.settings.shingleSize,
      numHashes: this.settings.numHashes
    };
  }
  async runScan() {
    new import_obsidian.Notice("RECLAIM: Scanning vault\u2026", 2e3);
    const result = await scanVault(this.app.vault, this.scanOptions());
...
var ReclaimReadinessView = class extends import_obsidian.ItemView {
  ...
  render() {
    ...
    if (this.plugin.settings.showDimensionBreakdown && r.subScores) {
      const dims = container.createDiv("reclaim-dimensions");
      const dimensions = [
        { label: "Uniqueness", value: r.subScores.uniqueness, weight: 30 },
        { label: "Structure", value: r.subScores.structure, weight: 25 },
        { label: "Density", value: r.subScores.density, weight: 20 },
        { label: "Title Quality", value: r.subScores.titleQuality, weight: 15 },
        { label: "Freshness", value: r.subScores.freshness, weight: 10 }
      ];
      for (const d of dimensions) {
        const row = dims.createDiv("reclaim-dimension-row");
        const label = row.createEl("span", { text: d.label, cls: "reclaim-dimension-label" });
        const barWrap = row.createDiv("reclaim-dimension-bar-wrap");
        const bar = barWrap.createDiv("reclaim-dimension-bar");
        ...
        if (d.value >= 80)
          ...
          bar.addClass("bad");
        const val = row.createEl("span", { text: `${d.value}`, cls: "reclaim-dimension-value" });
      }
    }
...
var ReclaimSettingTab = class extends import_obsidian.PluginSettingTab {
  ...
  display() {
    ...
    new import_obsidian.Setting(containerEl).setName("Show dimension breakdown").setDesc("Show per-dimension score bars in the readiness panel.").addToggle((t) => t.setValue(this.plugin.settings.showDimensionBreakdown).onChange(async (v) => {
      this.plugin.settings.showDimensionBreakdown = v;
      await this.plugin.saveSettings();
      this.plugin.updateReadinessView();
    }));
    containerEl.createEl("h3", { text: "Scanner thresholds" });
    new import_obsidian.Setting(containerEl).setName("Near-duplicate threshold").setDesc("Minimum Jaccard similarity (0.50\u20130.95) to flag near-duplicates. Lower = more sensitive.").addSlider((slider) => slider.setLimits(0.5, 0.95, 0.01).setValue(this.plugin.settings.nearDupThreshold).setDynamicTooltip().onChange(async (v) => {
      this.plugin.settings.nearDupThreshold = v;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Shingle size").setDesc("Number of words per shingle (5\u201310). Smaller = more granular, more noise.").addSlider((slider) => slider.setLimits(5, 10, 1).setValue(this.plugin.settings.shingleSize).setDynamicTooltip().onChange(async (v) => {
      this.plugin.settings.shingleSize = v;
      await this.plugin.saveSettings();
    }));
    new import_obsidian.Setting(containerEl).setName("Hash count").setDesc("Number of MinHash signatures (16\u201364). Higher = more accurate, slower.").addSlider((slider) => slider.setLimits(16, 64, 1).setValue(this.plugin.settings.numHashes).setDynamicTooltip().onChange(async (v) => {
      this.plugin.settings.numHashes = v;
      await this.plugin.saveSettings();
    }));
  }
};
